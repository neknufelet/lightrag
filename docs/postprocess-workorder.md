...
| id | 檢查 | 門檻 | 擋什麼 |
|---|---|---|---|
> ⚠️ **這張表是「當初的設計」，其中兩條在實作時被刻意改掉了**（2026-08-04 盤點發現，
> 文件一直沒跟上）：**V5 的 IDF 過濾**與 **V12 的數字分母**。兩處都已在下方標出，
> 理由寫在 `scripts/pp/vlm.py` 的註解裡——**以程式為準**。
> 這正是「同一條裁決散在散文裡、兩個版本同時存在」的實例，見 `docs/log_20260803.md`。

| V1 | `finish_reason == "stop"` | hard | 截斷 |
| V2 | 原始輸出以 `</table>` 結尾 | hard | 截斷 |
| V3 | 不含 `<img` | hard | 數學被換成圖片 |
| V4 | `LEAK` 不命中 | hard | prompt 洩漏 |
| **V5** | **alpha recall**：gt 的 `[A-Za-z]{3,}` 小寫集合在 HTML 的召回。~~經文件內 IDF 過濾~~ ⚠️ **實作後刻意不做 IDF**（`scripts/pp/vlm.py:196-201`）：那道與 V8 負向控制重複，且在這類文件上正好反向——會剔除 impedance／orifice／partition 這些「多數鄰近表都出現」的詞，而那正是唯一可比對的散文，留下的全是數學殘骸，召回必然掉到 0 | ≥ 0.70 | 漏抄 |
...
| **V12** | **分母下限**：gt 唯一 alpha token < 8 → 標 `unverified`。~~或唯一數字 < 6~~ ⚠️ **實作後刻意只看 alpha**（`scripts/pp/vlm.py:180-190`）：這類數學表的數字幾乎都是下標的 0/1/2，唯一數字只有 4–5 個，照原設計 10 張裡 9 張會被判 `unverified`＝整組閘門作廢、全部退回人工。**安全但無用。** 數字不足改為只讓 V6 不適用 | hard | **真空通過**：純數字表、純符號表在空集合上召回率恆為 1.0 |
...
## 附錄 B：穩態運作（2026-08-01 補）

本工單 W0–W12 全部為「390 份初始批次載入」設計。日常是一次加一兩篇，設計重點不同。

### B.1 批次與穩態的差別

批次刻意把階段拆開（全部解析 → 全部檢查 → 全部修補 → 才抽取），是為了讓偵測器或
過濾規則的錯誤在便宜的階段被吸收，不必拖著幾週的 LLM 時間陪葬。**單篇沒有這個攤提
效果** —— 從頭到尾 40 分鐘，錯了重跑即可。所以穩態應為一條指令跑完，任一閘門不過
即停。

閘門行為必須反轉：批次時人在旁邊看，可以「記錄後繼續」；穩態沒有人看，必須
**停下來等人**。寧可一份文件卡住，也不要讓有問題的內容默默進圖譜 —— 進去很難清，
生產 workspace 至今仍留著 2,776 個結構垃圾實體與 18 個 prompt 洩漏實體。

### B.2 W13 — `addrag` 單篇流程

```
addrag <pdf>
  compat-check → 解析 → parse-check（ERROR 即停）→ postprocess
  → verify → 抽取 → verify --deep
```

人工抽驗改為「列出本次所有修補表，操作者決定是否逐張看」。單篇修補量通常 0–2 張，
逐張看的成本遠低於維持一個抽樣比例。

### B.3 W14 — 漂移偵測（cron）

穩態的主要風險從「規則寫錯」變成「環境漂移」，而且兩種都是靜默的：

| 漂移 | 症狀 | 偵測方式 |
|---|---|---|
| LightRAG 升級改快取契約 | 修補被丟棄、雜訊回來 | `compat-check.py` 每日 cron |
| MinerU 雲端換模型 | 解析品質緩慢下降 | 金絲雀文件定期重解並比對基準 |
| MinerU token 過期 | 解析失敗 | `compat-check.py` 的 A-21（<14 天即警告） |

金絲雀用 `C Equivalent Networks.pdf` —— 它同時涵蓋空表格、數學密集表格、高比例頁眉，
是目前基準數字最完整的一份。**這是唯一能發現 MinerU 悄悄換模型的方法**，因為那不會
產生任何錯誤訊息。

`compat-check` 必須排 cron 而非只在加文件時跑：外部變動不會挑你加文件的日子發生。

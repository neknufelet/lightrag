...
supersedes: ""
superseded_by: ""
summary: "做不做都不影響新庫上線的待辦。從 NEXT.md 分出來，上線之後再撈回去。"
---

# BACKLOG — 以後再做

**這裡的每一條都不擋新庫上線。** 現在這一輪的在 [NEXT](NEXT.md)。

**怎麼用**：NEXT 清空、或某條忽然變成擋路的，就把它搬回 NEXT。**不要兩邊都留**
——同一件事在兩個地方就會漂移，而漂移不報錯（見 [knowledge-routing](knowledge-routing.md)）。

規矩與 NEXT 相同：做完的**刪整行**；`→` 後面是完成的判準；理由外引
（知道但不修 → [KNOWN_ISSUES](KNOWN_ISSUES.md)；裁決 → [decisions/](decisions/)）。

> 2026-08-19 從 NEXT.md 分出來。當時 NEXT 有 49 條、93 行（上游 A02 的上限是 80，
> **已經 hard 失敗而沒有人看到** —— pre-commit 不跑 `standards-check`）。
> PO 原話：「我看 next.md 怎麼還這麼多事情」。

## 拆章（設計見 [rebuild-v3-design](rebuild-v3-design-20260816.md)）
...
- ⬜ 50 份舊文件的處理計畫沒有 `title`／`refs` 段（舊版程式跑的，2026-08-17 實測，全部已 `indexed`）　→ 重跑計畫。⚠ **隨舊庫一起作廢**，新庫不會有；留著是因為它記著「舊版程式跑出來的計畫會缺段」這件事
...
- ⬜ pre-commit 不跑 `standards-check`　→ 2026-08-19 實測 A02 已經 hard 失敗（NEXT.md 93/80）而沒有任何訊號
...
- ⬜ **第八條候選：檢查上線前要證明它分得出東西**（血淚：`parse-check` 的 WARN 佔 285/319）

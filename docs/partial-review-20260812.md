...
supersedes: ""
superseded_by: ""
summary: "每一處都看過原文才下的判定；兩處疑義裁出原圖看像素解決。71 處全部是 ∂。改法、不能改的東西、以及還抓不到的一類，都寫在這裡。已於 2026-08-12 18:33 落地，探針歸零；知識庫要等 reindex。"
---
...
✅ **2026-08-12 18:33 已執行**（見文末），探針現在報 0 處。知識庫要等 reindex 才會反映。
...
**知識庫還沒有反映這次改動。** 改字與重建索引是兩件事：

```
改字      不花錢，而且不管以後重不重抽都不會白做      ✅ 已做
reindex   會白做的那一步 —— 若之後決定重抽，兩者應該併做   ⬜ 等 PO 決定時機
```

...
✅ **2026-08-13 已進索引**：十份一份一份 reindex，索引裡的誤讀 143 → 0（見文末）。
...
重蓋指紋之後，一份一份 reindex，**十份全部成功**：

```
索引裡的文件: 259
索引裡還有誤讀 token 的文件: 0  合計 0 個      （動手前 143 個）
```

第一份（`2017 - Optimal sound-absorbing structures`）逐項驗過，證實
**指紋有效時 reindex 不會重新解析**：

```
                動手前            動手後
size            109024            109024
mtime           08-12 18:33       08-12 18:33      ← 一個位元都沒動
sha256          d0b2bf3a3d32feb8  d0b2bf3a3d32feb8
帶原文的項目     42                42
指紋對得上       —                 True
索引裡的誤讀     2                 0
```

⚠ 中途發現：`backup-cold.sh` 的指紋**只看資料庫**，看不到磁碟上的改動
（重蓋 manifest 不會動到 DB），於是還原點被判定「沒有新東西」而跳過。
要在改完解析結果之後建還原點，得帶 `--force`。

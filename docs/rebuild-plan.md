...
**階段 3 完成（2026-08-03，主線獨立驗收）：**

20 份分四批索引，**processed 20／failed 0**，合計 3 小時 58 分。嵌入實際約
US$0.15（遠低於預估的 $1）。

- **A-25 自動轉正**：`chunk_top_k=2 → 2 個、=8 → 8 個（母體 20 份已索引）`，
  一行程式沒改。compat-check 135 項 hard 0／soft 0／**驗不了 0**——
  三態化設計的最終驗證通過。
- **體檢表 160 格全部填滿**：通過 151、fail 9、驗不了 0、未設定 0。
  fail 9＝3 份 waiver（41598／C 的 coverage、N Flow 的 equations）
  ＋6 份接地可疑率 >5%。
- **v155 → v2**：chunk 512→510、實體 7,191→7,211、關係 +1.2%、
  **含掉字 chunk 86→27（−69%）**、雜訊字串（相異）670→13。
  逐份無異常，最大變動都在無快取重跑的抽樣變異內。
- 檢索：20/20 可及（通用種子只命中 8 份，度數集中在 C／K／N Flow；
  逐份改用「該份獨有、度數最高」的圖標籤後全數回得到）。
  主線另自行打了一次 hybrid 查詢，回 44 實體／129 關係／3 chunk，內容對題。

**階段 3 學到的事：**

- **多 workspace 共存時，每一句 SQL 都要帶 workspace 條件。**
  `extract-check.py` 的三句 SQL 都漏了，於是 v155 與 v2 的同一份 PDF
  （`file_path` 相同）被併成一列——數字看起來正常（約兩倍）、不報錯，
  但**翻轉了三份文件的閘門判定**。單一 checkout 時代這個 bug 不可觀測，
  與階段 0 的「容器名寫死」是同一個根源：第二個實例存在的那一刻才引爆。
  修法的驗證方式值得複製：**修完拿舊 workspace 重跑，要能逐位元重現
  歷史數字**（實測回到 CLAUDE.md 記載的 3.2%），偵測器才算驗過。
- **「可疑」不等於「幻覺」，形狀要逐份看。** 六份 >5% 全部查過，沒有一個
  是憑空捏造，分兩族：符號→概念命名（裸符號被命名為 `Coefficient Ta`
  這類）與概念→引用文獻（作者縮寫、期刊全名 vs 縮寫）。
  這**推翻了 v155 時代對 K Muffler 的歸因**——92 個可疑裡只有 1 個是
  引用文獻型，根因是 `SYMBOLIC_RATIO=0.35` 把同一族切成兩半。
- **通用檢索種子會被大文件壟斷**：度數最高的標籤集中在少數幾份，
  用它們當種子時 12 份一次都沒被命中。驗「每份都檢索得到」必須用
  **該份獨有**的種子，否則等於宣稱驗過沒驗的東西。

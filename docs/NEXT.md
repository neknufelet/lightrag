...
**現在的狀態**：`compat-check` hard 0、soft 0、驗不了 3（空庫本來就問不出答案的
那三條）。四個容器健康。資料根空的，等進料。

- ⬜ 把 300 多篇進料進來　→ 審核台的「已進知識庫」數字對得上 PDF 份數

  ⚠ **MinerU 的 token 2026-09-04 到期，剩不到一個月**，而這批要全部重新解析。
  先確認額度撐得住 300 多篇，不夠的話先換 token 再開始 —— 解析到一半斷掉最貴。
  ⚠ canary 基準要重新建（舊的 27 份已不存在），否則改規則時沒有安全網。

- ⬜ 進完之後跑 `extraction-profile.py stamp`　→ A-32 從「驗不了」轉綠
  （現在沒有紀錄檔，舊的那份在清庫時刪掉了 —— 它描述的圖譜已經不存在）

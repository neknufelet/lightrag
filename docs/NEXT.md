...
- ✅ 進料台接拆章：讀目錄 → 提方案 → 人勾掉不要的 → 才切才解析　→ **2026-08-17 完成並在真書上走完**（Rossing《The science of sound》，32 章 748 頁，逐份驗過頁數；舊庫測試，驗完清掉）
- ⬜ `pdf_splitter.py` 的測試從 6 增到 10（`extract_pages` 那層有了）　→ 剩「無目錄三策略」與「超長章子切」沒有自己的測試
- ⬜ 附錄編號 900 會撞第 9 章（`appendix_idx` 從 0 起算）　→ 改基底或改起算，現有檔名的影響先想清楚
- ✅ 定確認清單的介面與存檔格式　→ **2026-08-17／18 完成**，見 [confirm-list-design](confirm-list-design-20260817.md)
- ✅ 確認清單的「放行」那半　→ **2026-08-18 完成並在 dker 實跑驗過**（沒確認的停在 planned，紀錄寫著理由）
- ⬜ 剩下 117 項還沒歸類的規律（2026-08-19 量，全庫重算、當作沒人確認過）：投稿日期漏網 13、ResearchGate 下載頁 6、單獨作者名約 30、作者單位 6、`ARTICLE INFO` 4、掃壞的字 4、其餘一份一份不同　→ 各自變成規則或明確裁定「就是要人看」
- ⬜ 「我先作答、你再對答案」那支　→ 前 20 份藏我的答案（誠實基準），之後才顯示
- ⬜ 人名那條漏掉沒有縮寫點的寫法（"Michael Stinson"）　→ 量新庫的分佈再決定要不要放寬
- ⬜ 佐證數字接上來源（`render_confirm(evidence=…)` 現在永遠是空的）　→ 畫面上看得到「這份參考書目佔 N%，平常 M–K%」
- ⬜ 決定要不要另做一頁覆核「規則有把握直接丟」的那些　→ PO 裁（現在只在每一份底下點得開）
- ⬜ 三個 skill 改網址與 workspace　→ 新庫獨立
- ⬜ `mount-guard`／`backup-cold.sh` 加上新 stack　→ 新庫也被守著、也有備份
- ✅ 決定新 workspace 叫什麼　→ **`rag_acoustic`**（PO 2026-08-17）
- ⬜ 修外掛 `KI-016`：對 PDF 附件那一列送出會繞過選片規則　→ 那條路徑有一條「修之前是紅的」新測試（上線前必修，路標第 1 步）
- ⬜ **新庫起來之後**走一次還原　→ 列得出快照、抽得出一個檔（`(未驗)` 至今沒做過）。⚠ 舊庫那一次 PO 2026-08-19 裁定不做（「我大概也不會回來了」），但**機制本身新庫照用**，不能跟著作廢

- ⬜ 50 份舊文件的處理計畫沒有 `title`／`refs` 段（舊版程式跑的，2026-08-17 實測，全部已 `indexed`）　→ 重跑計畫，否則確認清單算不出它們有沒有要看的項目
...
- ⬜ `compat-check` 的 A-38 沒有 `ZOTERO_API_KEY` 時「這條沒跑」被算成 soft 不過　→ 分得出「驗不了」（金鑰 2026-08-19 已補上、A-38 實跑 True，但**沒金鑰時仍然回 False**，缺口還在）
...
- ⬜ `lightrag-snapshot` 這個 plan 還指著 `/userdata/data/lightrag`　→ 新庫起來後改指
...
- ⬜ `intake.py` 4300+ 行、180 個函式（2026-08-17 量）　→ 拆開。**是獨立工作，不要順手做**：它是整條生產線的心臟，拆到一半功能會卡住。拆章那批新程式已刻意放 `scripts/chapters/`，只在這支留接線
...
- ⬜ 決定要不要加「`/opt/stacks/lightrag/` 底下不准有第二個含金鑰的檔」這條檢查　→ PO 裁（2026-08-19 清掉 5 份躺了十天的舊備份，靠的是人看到，不是機器擋）

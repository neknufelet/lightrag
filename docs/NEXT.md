...
- ⬜ 🔴 **接地檢查的 `norm` 把中文抹成空字串**　→ 中文文獻不再假紅燈

  ```
  norm("微穿孔板") = ''        `[^0-9a-z]` 把非英數字全換成空白
  ```

  `2025 - Recent advancements in sound-absorbing materials`（庫裡唯一的中文文獻）
  233 個實體裡 95 個被判成「模型編出來的」，而 `微穿孔板` 用 SQL 查得到就在原文裡。
  **表上那面紅燈現在正在誤導人。**

  ⚠ 改 `norm` 是一行的事，但它是**判準**，而那把尺剛寫了 259 格 —— 改完要重跑、
  重寫、重看。跟下面兩條一起處理。

- ⬜ `parse.coverage` 的判準　→ 決定「超標算不算壞」

  照現在的門檻會製造 13 個紅燈，而逐份查證 4 個**全是誤判**。

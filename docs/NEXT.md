...
- ⬜ 剩下 298 項還沒歸類的規律：掃壞的字 25、極短 46、參考文獻跑到頁眉 5、其餘 213（研討會名／廣告／網頁按鈕）　→ 各自變成規則或明確裁定「就是要人看」
...
- ⬜ 刪舊庫之前把 `records/` 與 `checks/` 保住　→ 336 個檔不在 git（實測），搬完 `comm` 比對是 0
- ⬜ 刪舊庫之前實際還原一次備份　→ 列得出快照、抽得出一個檔（`(未驗)` 至今沒做過）

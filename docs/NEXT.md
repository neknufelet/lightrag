...
分組本身做完並實測過了（設計與驗收在 [intake-batching-design](intake-batching-design.md)）。
下面是**跑起來之後才看得見的**，依「痛的程度」排。

- ⬜ **審核台每次都重算全部 PDF 的 sha256**　→ `/api/state` 回應 < 0.5 秒

  畫面每 3 秒問一次 `state()`，而它每一次都把 `library/`、`work/parsed/`、
  `inputs/`、收件匣底下**每一個 PDF 重新雜湊一遍**（`CandidateScanner._known_hashes`）。
  2026-08-10 實測：`/health` 0.001 秒、`/api/state` **5.1–5.5 秒**。
  它跟檔案總數成正比，會隨進料越來越糟，而且等於一直有一顆 CPU 在算雜湊。

  **內容定址本身是對的**（同一份文件改檔名還認得出來），錯的是每次都重算。
  修法：用 `(路徑, 大小, mtime)` 當快取鍵；任何一項變了就重算，正確性不打折。

- ⬜ **一批的尾巴是逐份跑 compat-check**　→ 整批驗證時間不隨份數線性成長

  抽取做完之後 `_extract_batch` 逐份跑契約檢查，2026-08-10 實測 **約 20 秒／份**，
  86 份的尾巴約 28 分鐘，而且每次都打一輪 Postgres。
  修法：**跑一次全庫的檢查，把 hard 失敗歸屬到各文件** —— `intake-reconcile.py`
  已經是這個形狀，抄它。

- ⬜ **intake 走完自動寫體檢表**　→ `ledger.py summary` 的份數跟知識庫對得上

  **「東西抽錯了要有第二次檢查」的機制早就寫好了，但沒有人在用。**
  `ledger.py` 是每份文件八道閘門的三態表（`pass`／`fail`／`驗不了`，而且
  `驗不了` 強制附理由）—— 知識庫上百份，體檢表只有 20 份舊語料的紀錄。

  它是**手動**的，所以一批幾十份進來不會有人去記。而 intake 走完一份時手上
  本來就有那些判定（`tables.review` 待查的表、`noise.held` 消音但保留的、
  `charts.dangling` 指不到檔的圖）—— 產生了、顯示一次、然後就沒了。

- ⬜ 現有文件補跑一次判定　→ 體檢表有母體可看

  不用重新解析、不花錢，逐份重跑 `postprocess.py plan` 把判定重算即可。
  **沒有這一步，上一條只對「之後進來的」有效。**

- ⬜ `health.running` 在整批抽取時是 False　→ 欄位語意與實際相符

  等索引那段跑在協調執行緒上，而那個旗標看的是 worker 佇列。
  ⚠ **畫面的自動重整不受影響**（它另外 OR 了「處理中有東西」，已驗），
  所以這是欄位語意瑕疵，不是使用者可見的 bug。

- ⬜ 把「判準會隨解析結果變動」寫進 [hard-rules](hard-rules.md)　→ 領域知識那節找得到

  頁面尺寸的判準現在問「**那張表需要裁圖嗎**」，而 MinerU 對表格的辨識不可重現
  （鐵則第 8 條）—— 同一份文件重抽之後可能從「過」變成「擋」。那不是規則不穩，
  但**現在只寫在程式註解裡**，下次遇到的人會困惑。

- ⬜ 裁圖改成逐頁換算　→ 「頁面尺寸一致」這條規則可以整條拿掉

  **根本解，但不急。** bbox 是每軸各自正規化到 0–1000，換算只需要「那一頁」的
  尺寸；而 `empty_table.plan(items, page_w, page_h)` 只收一組尺寸套用到整份文件。
  **那條規則存在的唯一理由就是這個介面。** 改成逐頁之後，橫向頁的表會被正確裁出來。
  ⚠ 這次 canary **可能真的會漂移** —— 前兩次改判準沒漂是因為只放寬了門，
  這次改的是算出來的座標。

- ⬜ 93 份新文件納入 canary 基準　→ `canary` 的「尚未納入基準」歸零

  要**逐條確認漂移都是想要的**再 `--update`，不要整批認可。

- ⬜ `docs/NEXT.md` 超過交接檔行數上限　→ `standards-check` 的 VERIFY-9-A02 回綠

  這是 `standards-check` 唯一的 hard 失敗，而且**我今天讓它更長了**。
  它不在每日排程裡，所以平常沒有人會看到。

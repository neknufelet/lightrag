...
- 🔴 **`reindex` 會把後處理修補洗掉，而且沒有人知道正確做法**　→ 找到能讓修補生效的路

  2026-08-12 21:26 實測：reindex 那 10 份之後，∂ 探針 **0 → 668 處**，
  `N Flow Acoustics` 帶原文的項目 **455 → 0**。**被洗掉的不只當次改的，
  是那份文件歷來所有的修補。** 靠 21:17 的還原點救回來（停機 69 秒，
  還原路徑第一次真的被走，有效）。

  ⛔ **在找到正確做法之前不要再跑 `reindex`。**
  已知線索：新文件走 `prepare`（解析 → 修補 → 掃描），修補是在**第一次進索引
  之前**套用的；已經在索引裡的文件目前沒有找到「讓修補生效而不重新解析」的路。

- ⬜ **∂ 誤讀已修進解析結果，但知識庫還沒反映**　→ 等上面那條有答案

  71 處已修、探針 0、逐處看過原圖（19/34，10 份文件與 8 種符號全覆蓋）。
  修正在磁碟上是好的，**進不了索引**。

  ✅ 已做（2026-08-12 18:33）：71 處逐處判讀、修進 `content_list.json`、探針歸零、
  canary 通過。原文在 `_pp_original_*`，時間戳 `2026-08-12T18:33:04+08:00`。
  逐處依據在 [partial-review-20260812](partial-review-20260812.md)。

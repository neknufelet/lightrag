...
## 重抽之前必須做完（見 [review-20260808](review-before-reextract-20260808.md)）

按「不做會怎樣」排序，不是按工作量。

- ⬜ `postprocess.py apply` 對 6 份實跑　→ 每份的「文獻」行顯示已消音
  （現在只跑過 `plan`。不跑就等於參考文獻照樣進索引，**這次重抽白費**）
- ⬜ 重新載入 `acoustics.yml`　→ 容器裡的指引含「2a. POSITION MARKERS」
  （規則 2、3 是套用之後才改的，容器跑的還是兩條的舊版）
- ⬜ pg_dump 備份　→ dump 檔存在且大小合理（沒有退路的操作不要做）
- ⬜ 決定 2017 那篇放不放行　→ 它 preflight 已通過，要進就趁這次
- ⬜ **記錄「這批是用哪版規則抽的」**　→ compat-check 能比對規則檔雜湊與圖譜的

  **這是唯一「現在不做、之後補不回來」的**：重抽當下才知道用的是哪版規則，
  事後只能推測。解析那側有 `options_signature`（A-11 在守），抽取這側沒有對應物
  ⇒ 之後新進的文件用新規則、舊的用舊規則，圖譜混著兩代而**沒有紅燈**。

## 從這一段檢討出來的規範（要有執行者）

- ⬜ 秘密不得經由「整包輸出」離開系統　→ 升上游 BASELINE
  （今天第三次外洩，而正解早就寫在 `cairn/secrets-handling.md`）
- ⬜ 「有權威來源時不得自己重算」　→ 升上游 BASELINE
  （今天三次：容器要不要重建、設定雜湊、chunk token 數，自己算的全錯）
- ⬜ 「不要寫死 localhost」要有執行者　→ grep 得到就紅
  （`.env.example` 早就寫了這條規矩，我新寫的腳本照樣違反）

...
- ⬜ **放行階段失敗不該丟掉有效的解析結果。** 狀態機裡 `failed` 是終點，於是「暫存區
  不乾淨」「pipeline 忙碌」這種**跟解析無關**的失敗，也只能整個 `reset` 回候選重新解析
  （2026-08-08 四篇論文實際踩到）。`failed_parse` 當終點是對的，但放行階段的 `failed`
  應該能退回 `planned` 重試——解析產物還是好的，驗 sha 一致就可以再放行一次
- ⬜ **`reset` 會清掉 library／parsed／inputs 三處的產物**，所以退回候選＝解析全部重來。
  分開語義之後這條也會跟著改善

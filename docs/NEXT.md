...
- ⬜ 新文件納入 canary 基準　→ `canary` 的「尚未納入基準」歸零
  ...
  ✅ **漂移的部分 2026-08-16 稍晚處理完了**（`16be851`）：`C Equivalent Networks`
  的 `review: 1 → 0` 與標題頁規則放寬造成的 7 處，逐處確認過都是想要的並寫進基準。
  **做法留著給下次用**：在 dker 跑 `--update` 取得結果 → **只挑原本就在基準裡的那幾份
  寫回** → 隨即 `git checkout -- tests/canary-baseline.json` 還原 dker 工作區。
  這樣「認可漂移」與「吃進新文件」就分開了。

  ⬜ **剩下的是「新增」那 247 份**，仍然沒有納入基準（基準維持 70 份）。
  要納入得逐份確認，不得整批 `--update`。
...
- ⬜ **第八條候選：檢查上線前要證明它分得出東西**　→ 有執行者才升格

  **有執行者，機器判得出來**：跑過現有母體、印出分佈，單一格超過門檻就不得上線。
  血淚（2026-08-16）：`parse-check` 的 WARN 在 317 份裡佔 283 份（89%）。
  它上線、每天跑了好幾個月，**沒有任何東西發現它不分辨任何東西**。
  ⚠ 這條是 PO 當天糾正我之後才成形的 —— 我原本歸因成「加檢查太便宜」，
  他指出「那不就表示檢查沒寫好嗎」。**是他對。** 病不在便宜，在沒有人驗鑑別力。

- 🟡 **觀察（還不能寫成條文）：鬧鐘不能代替脈絡**

  > 每當想加一個檢查來防止某件事再發生，先問：**如果那個人在動手的那一刻就看到
  > 這句話，還會出事嗎？** 會 → 才加檢查。不會 → 把話貼到那個位置去，不要加檢查。

  **執行者：目前是「沒有人」**，所以照 BASELINE 自己的規矩不得寫成條文
  （「新規則答『沒有人』者不得寫成條文，先進觀察區，配到執行者才升格」）。

  兩次血淚都不是「沒有檢查」，是**脈絡沒到眼前**：
  ```
  pp/apply.py 檔頭一字不差預告了「reindex 會洗掉修補」 → 寫的人沒讀（08-13）
  coverage-check --doc 會印「已查證於某日…根因…」      → 我沒問它，自己推，推錯（08-16）
  ```

  PO 2026-08-16 的三個補充（他的原話比我的框架準）：
  - 「**靠人沒屁用，直接跳出來比較好，或是中斷相關指令**」
    ⇒ 與 BASELINE「人的審查不算執行者」一致。候選機制是 hook。
    今天 `guard-command.py` 擋了兩次、兩次都對 —— **這個專案唯一被證明有效的機制**。
  - 「做一個動作就直接觸發一個機制，同一個機制只是內容不同」⇒ 一個派發器、多條規則。
  - 「到什麼階段就按一次，把所有東西檢查一次，包裝成 skill」⇒ 收尾盤點那一層。

  ⇒ **分工判準：機器判得出來且誤判便宜 → hook 當場擋；誤判很貴 → 只印警告；
  判不出來 → 收尾 skill 給人看。**

  ⚠ **下一步不是把這條寫得更漂亮，是找它的執行者。** 沒有執行者就只會變成
  第 88 條沒人執行的規則（上游盤點：173 條教訓裡 51% 的執行者是「沒有人」）。

## 第 7 批：備份（3 條，PO：資料庫都建好再開始）

- ⬜ 關掉 backrest 備份 `/data/rag` 的排程（那目錄已廢除，見 ADR-0003）
- ⬜ **backrest 掛了整個 `/data`，硬碟不見時它看得到空的 `/data/lightrag`**
  → 確認它會不會拿空目錄覆蓋好的備份

  2026-08-16 實查 `docker inspect backrest`：`/data -> /userdata/data`。
  `mount-guard` 保護得了我們的四個容器（硬碟不見就停），**保護不了 backrest**。
  ⚠ **只知道它看得到，不知道它會不會蓋掉 —— 那是推測不是事實。**
  它是別人的容器（CLAUDE.md：不要碰），要看設定得先問過 PO。

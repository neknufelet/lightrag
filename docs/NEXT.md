...
- ⬜ 「同一個 ∂ 被讀成兩個不同字母」還沒數過　→ 知道有多少

  分子 `D̂`、分母 `Ô` 這種，**上下同形永遠看不到**。已知三例記在同一份文件裡。
...
- ⬜ `parse.coverage` 的判準　→ 決定「超標算不算壞」

  **2026-08-13 查清楚了一部分**：13 個紅燈裡有 1 個是量測 bug（軟連字號，已修，
  見 `898b478`），修完剩 12 個。剩下那 12 個的漏詞長這樣 ——
  `fdtd`／`mode`／`real`／`imag`／`amplitude`／`driver`／`seat` ——
  **那是圖裡的座標軸標籤與圖例**，本來就不在文字裡，後處理救不了。
  ⇒ 建議「會叫但不擋」，理由不再是「抽查都是誤判」而是
  **「剩下的都是圖內文字，而圖內文字本來就檢索不到」**。
  真要根治是讓 coverage 把 image 型別的區域分開報，那是另一件事。

- ⬜ `parse.checks` 的 WARN 怎麼記　→ 決定

  WARN 占 225/257（88%），沒有鑑別力。ERROR→fail、OK→pass 沒有爭議。

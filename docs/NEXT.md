...
- ✅ **`CHUNK_P_SIZE` 維持 2000。** 2026-08-08 並排實測：砍到 1000 讓 chunk 207→405
  （抽取成本 +96%），而檢索品質在雜訊範圍內。細節見 `cairn/retrieval-tuning.md`
- ✅ **embedding 用本地 BGE-M3 @1024。** 維度不影響品質（3-large 的 3072／1536／1024
  三者無差異），BGE-M3 與 OpenAI 品質平手，換的理由是少一個外部依賴
- ✅ **workspace 不改名。** `_v2` 難看但功能上只是字串，而改它要動 `backup-cold.sh`
  的容器名、`systemd-units.py` 的預設值、三個 skill 的 8 處 URL、三個測試檔——
  波及面大、價值低

...
## 🔴 進料分組上線之後的待辦（2026-08-10）

分組做完並實測過了（[intake-batching-design](intake-batching-design.md)）。
下面是**跑起來之後才看得見的**，做完的已刪。

- ⬜ **intake 走完自動寫體檢表：只寫得起兩格**　→ 六格都有值

  已接：`pp.preflight`、`pp.tables`（計畫一判完就寫，被 preflight 擋下的也會留紀錄）。
  **其餘四格留空 —— 沒跑過的閘門填 `pass` 就是說謊。** 要填滿得把
  `coverage-check`／`parse-check`／`extract-check` 的結果也寫進去，
  而前兩隻已經在每日排程跑了，只差把結果接進表。

- ⬜ 現有文件補跑一次判定　→ 體檢表有母體可看

  不用重新解析、不花錢，逐份重跑 `postprocess.py plan` 即可。
  **沒有這一步，上一條只對「之後進來的」有效。**

- ⬜ **`extract-check` 沒有接進任何自動流程**　→ 決定它掛哪裡

  2026-08-10 實測全庫要一個多小時，接每批收尾等於每批多一小時，
  放每日排程則是天天跑一個不會天天變的東西。目前靠 `kb-health-check` skill 手動叫。

- ⬜ 那兩塊碎字元消音　→ `parse-check` 的 ERROR 歸零

  2012 CH8 第 23 頁、~~C Equivalent 第 64 頁~~，各約 100 多字元。**旁邊的表格都抽得
  很好**，那兩塊是表格標題被誤讀的殘影，沒有值得救的內容 —— 消音即可（鐵則 2，
  可還原可查帳，零成本）。⚠ 現有規則抓不到：它們是 `text` 型別、有內容、
  不是 header/footer。要加一條「碎字元」偵測。

  ✅ **C Equivalent 那塊 2026-08-16 已經修掉了**，而且不是靠消音——人工裁定
  `#540` 早就存在（`pdftotext` 逐字打的散文），只是從來沒進過 dker。合併之後
  `apply --commit` 一次就套上去，`parse-check` 的 ERROR **3 → 2**。
  ⇒ 現在剩下的 2 份，08-16 逐份實跑確認過名字：
  `2012 -Combined wave…_CH8`（**是 CH8，不是 CH7** —— 總表那行檔名被截斷，
  CH7 是 `coverage-check` 印的、不是同一支尺）與 `2020 - Folded metaporous…`。

- ✅ **`reindex` 洗掉修補的原因找到了，是我自己的 bug**（2026-08-13 修）

  `scan-partial --repair` 直接改 `content_list.json` 卻沒更新 `_manifest.json`
  的指紋 → `is_bundle_valid()` 回 False → 掃描時當成壞快取重新解析 → 覆蓋掉修補。
  **`reindex` 沒有壞，文件寫的流程也沒有錯。**
  ⚠ `pp/apply.py` 的檔頭一字不差地預告了這件事，我沒讀就另開了一條寫入路徑。
  已修：指紋維護抽成 `apply.restamp_manifest()`（唯一擁有者），蓋不了章就不動內容。

- ✅ **∂ 誤讀 71 處已經進索引**（2026-08-13）

  索引裡的誤讀 **143 → 0**（全庫 259 份逐份用探針自己的 `repair_text` 數過）。
  逐處判讀與驗證在 [partial-review-20260812](partial-review-20260812.md)。

- ⬜ `backup-cold.sh` 的指紋只看資料庫，看不到磁碟上的改動　→ 改完解析結果也會備份

  重蓋 manifest 不會動到 DB，於是還原點被判「沒有新東西」而跳過 ——
  而那時磁碟上的東西**確實變了**。2026-08-13 是靠 `--force` 繞過的。
  ⚠ 這條的形狀跟今天其他幾個一樣：**它報的母體不是真的母體。**

- ✅ 「同一個 ∂ 被讀成兩個不同字母」2026-08-16 直接查索引數過了，全庫 2 份中招，
  修掉 1 份（見 [LOG](../cairn/LOG.md)）。「誤讀 143 → 0」是探針自己量的，
  **探針看不到這幾種**。
  ⚠ 查它**不能用 `LIKE`**：PostgreSQL 的 `LIKE` 預設拿 `\` 當跳脫字元，
  `'%\hat { \sigma }%'` 實際比對 `hat { sigma }`，**回 0 而且不報錯**。用 `strpos`。

- ⬜ `O Analytical and Numerical Methods in Acoustics.pdf` 的 1 處 ∂ 誤讀
  → 索引裡的 `hat_mathcal_O` 歸零

  全庫最後一份中招的。08-16 實測 1 個 chunk、`full.md` 裡 1 處。原文長這樣：

  ```
  \int \displaylimits _ { \hat { \mathcal { O } } \mathbf { n } ( y ) }
                     ^ { \hat { \mathcal { O } } \mathbf { p } _ { 1 } ( y ) }
  ```

  **是 `∂p₁/∂n` 被讀成積分的上下限 —— 連結構都壞了**，不只是字母換掉。
  這份**沒有任何人工裁定可用**，要人看 PDF 才修得了。
  ⚠ **不要靠放寬上下同形去抓** —— 放寬等於「任何 accent 對 accent 的分數都算導數」，
  而真的比值就是長那樣（Favre 與 Biot 兩個誤報都是這個形狀）。

- ⬜ `latex_fix` 修不掉 ∂ 誤讀，而文件說它可以　→ 決定要不要補規則

  08-16 實測：拿 apply 之前的原文跑 `latex_fix.fix_one`，19 個真的改了內容的
  裁定檔**重現得出來 0 個**；三種樣式規則跑之前之後一樣多（5→5、7→7、1→1）。
  ⇒ `verdicts/README.md` 原本說那 163 個 `.txt` 可再生，對其中 19 個是錯的
  （已更正）。**這 19 個是不可再生的人工成果。**

- ⬜ `latex_fix.fix_partial` 與 `scan-partial --repair` 是兩條路　→ 合成一條

  前者認**字元清單**（只認 `\hat{\sigma}` 那幾種），後者認**位置**（剖析出來的算子位）。
  2026-08-12 沒有動前者，因為它那 190 處是驗過的，動它要重驗。
  ⚠ 但兩條路遲早會漂移 —— 這個專案已經被「兩條路」咬過一次（十二道閘門的 V1／V2）。

- ⬜ `health.running` 在整批抽取時是 False　→ 欄位語意與實際相符

  等索引那段跑在協調執行緒上，而那個旗標看的是 worker 佇列。
  ⚠ **畫面的自動重整不受影響**（另外 OR 了「處理中有東西」，已驗）。

- ⬜ 把「判準會隨解析結果變動」寫進 [hard-rules](hard-rules.md)　→ 領域知識那節找得到

  頁面尺寸的判準現在問「那張表需要裁圖嗎」，而 MinerU 對表格的辨識不可重現
  （鐵則 8）—— 同一份文件重抽後可能從「過」變成「擋」。現在只寫在程式註解裡。

- ⬜ 裁圖改成逐頁換算　→ 「頁面尺寸一致」這條規則可以整條拿掉

  **根本解，但不急。** bbox 是每軸各自正規化，換算只需要「那一頁」的尺寸；
  而 `empty_table.plan(items, page_w, page_h)` 只收一組套用到整份文件 ——
  **那條規則存在的唯一理由就是這個介面。**
  ⚠ 這次 canary 可能真的會漂移（前兩次改判準沒漂是因為只放寬了門，這次改座標）。

- ⬜ 新文件納入 canary 基準　→ `canary` 的「尚未納入基準」歸零
...
**同一個形狀今天出現三次**，而且每一次的第一反應都是「知識庫有問題」：

```
中文文獻「英文問搜不到」        量錯 —— 拿五道專門題去問一篇綜述
中文題「檢索差 63–86%」        量錯 —— 打分模型有語言偏心，送英文進去就一樣好
中文文獻「95 個幻覺實體」      量錯 —— 接地檢查的正規化把中文抹光
```

⇒ **這個庫的檢查工具全都是為英文語料寫的。** 庫裡現在有中文文獻了，
每一支尺都會對它產生假警報。前兩個已經修掉，第三個還在。

- ✅ **接地檢查的 `norm` 已經修好**（`da0fac1`，2026-08-13 覆驗）

  `_NORM` 的字元類補進假名／漢字／諺文區段。實測：
  `norm("微穿孔板") = '微穿孔板'`、`grounded("微穿孔板", 中文原文) = True`。

- ✅ 兩個門檻 2026-08-16 定案 → [ADR-0007](decisions/0007-parse-gate-thresholds.md)

- ⬜ 把兩格接進 `intake` 的收尾　→ 六格剩兩格空著

  **判準定了，接線還沒做。** ADR-0007 只裁決「什麼算壞」，把
  `coverage-check`／`parse-check` 的結果寫進體檢表是另一件事。

- ⬜ 每日體檢要分得出「擋流程的紅」與「只是提醒的紅」　→ `parse_rc`／`coverage_rc`
  不再天天紅

  ADR-0007 刻意沒動兩支工具的 exit code，所以每日體檢照樣天天紅。
  ⚠ **不要用「把 exit 改成 0」解決** —— 那是消音，哪天真的壞了也沒人知道。

## 🟡 來源登記已上線，但沒有執行者（2026-08-13）

來源改成查 `verdicts/source-map.json`（人核過的資料），檔名推論已刪。
判定過程 [source-review-20260813](source-review-20260813.md)。**剩下的都是「沒人守」。**

- ⬜ **新文件進來不會自動登記**　→ intake 放行時把來源宣告寫進登記檔

  現在只有這一輪 259 份是登記過的，**下一批進來全部落在 unknown**（整份不進比對，
  方向安全但會安靜地少報）。`intake.py` 的 job 紀錄本來就有 `source_name`／
  `source_key`，而 `_configured_source_roots()` 讀碼確認支援 `<書名>/raw/*.pdf`
  巢狀目錄 —— 但 **`INTAKE_SOURCES` 在實機上根本沒有這個鍵**，259 個 job 全走扁平
  inbox。打開它是動生產路徑，要自己排一輪測試。

- ⬜ 獨立性稽核：兩個「宣告獨立」的來源之間公式重疊率異常高就點名　→ 分組漂掉有人叫

  **只提案不裁決**（用相似度自動合併＝把易腐觀察寫成規則，鐵則禁止）。
  ⚠ 偵測器自己要先驗：跑出來必須把已知的兩組（2026 hybrid ↔ 2024、
  2021 TD_DG ↔ 2019）重新找出來，找不到就是尺壞了。

- ⬜ `2021 TD_DG` 的 Paper I／III／… 對應庫裡哪幾篇　→ `same_work_as` 補齊

  它的 CH4 內文自承「based on Paper III」但沒說是哪一篇，要翻它的 paper list。

- ⬜ `2023 Perception-of-room-modes` 與 `2005 - Perception_of_Modal_Distribution_Metrics…`
  是不是同一作品　→ 查完決定要不要併

  這一輪的自動比對沒撈到（只讀了每章前三段文字）。**先留著不動，少報不假報。**

- ⬜ **標註表等人標**（19 題，[eq-label-review-20260813](eq-label-review-20260813.md)）
  → `--min-ratio` 才有資格從排序起點變成判準

  第一部分 3 題是三隻模型投不出多數的；第二部分 16 題按相似度分四段。
  標完凍結成 `verdicts/eq-labels.json`，同時當回歸基準（改骨架規則後必須還對得上）。

- ⬜ 那 51 組判「是同一條」的**沒有人工複核**　→ 抽驗一批

  只信三隻模型的多數決。模型在「不是同一條」那個方向實測 100% 準、零次亂點頭，
  但「是同一條」那個方向**沒有獨立驗證過** —— 而那是會製造假報的方向。

- ⬜ `pp/judge.py` 的表格裁判仍然零個呼叫點　→ 決定接哪裡或刪掉

  2026-08-13 發現。跟十二道閘門 V5–V12 同一個形狀（寫好的檢查沒被呼叫等於沒寫）。
  公式那條路已經共用它的 `ask_json()`，所以現在至少有一個呼叫者了 ——
  但它自己的 `ask()`／`negative_control()` 仍然沒有。

## 其他

- ✅ **進料已完成**（2026-08-13 覆驗）：收件匣 259 份 PDF、知識庫 259 列，對得上。
  ⚠ 下面那些注意事項留著 —— **下一批進料時仍然適用**。
...
✅ **放行併行做完了**（2026-08-10）：不是把 `apply` 挪到解析後，是把流程切成三段、
用單一協調執行緒讓改稿與抽取錯開。設計與實測在
[intake-batching-design](intake-batching-design.md)。
...
## 第 7 批：備份（3 條，PO：資料庫都建好再開始）

- ⬜ 關掉 backrest 備份 `/data/rag` 的排程（那目錄已廢除，見 ADR-0003）
- ⬜ **走一次還原** → 「停掉、換回目錄、啟動」確認可行（見 [backup-design](backup-design.md)）

  ⚠ 至今**沒有人走過**。要驗必須停服務、動資料根，挑有人在旁邊的時間，不要在進料途中。

- ⬜ **kbapi 不理 SIGTERM，被 SIGKILL 砍** → 停機窗從 66 秒掉到約 6 秒

  `docker stop -t 60` 的 60 秒是寬限期，自己結束就不會用滿 —— **kbapi 每次都用滿**
  （2026-08-10 兩次實測都是整整 60 秒，其他兩個容器 0～1 秒）。
  停機窗的 90% 是這一個容器，而且每次都是硬砍的。

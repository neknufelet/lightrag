...
## 🔴 審核台顯示的狀態是假的（2026-08-08 重抽時當場抓到）

- ⬜ 審核台對**自己送進去的**文件也要去問 LightRAG，不能只信自己的簿記
  → 外部改動索引之後（例如 `postprocess.py reindex`），畫面在一次重整內反映實況

  **現象**：重抽進行中，5 份文件在 `analyzing`／`processing`，而審核台顯示
  「已進知識庫 5、處理中 0」。同一個畫面上「樣本還太小（4 份）」與「已進知識庫 5」
  互相矛盾。

  **根因**（`/api/state` 實測）：
  ```
  sections.completed  4   ← 審核台自己記的 job 狀態（當初 indexed，從此不再問）
  foreign             1   ← C Equivalent Networks，它有去問 LightRAG，回報 processing
  ```
  **對「別人送進去的」會去問，對「自己送進去的」只信簿記。** 於是任何從審核台
  外面動索引的操作（reindex、手動刪除、重建）都會讓畫面靜靜地說謊，
  而且**沒有任何紅燈**——這正是今天一整天在修的那個形狀。

  ⚠ 這條的危害不只是難看：**審核台是本專案唯一的警報管道**（2026-08-08 PO 裁決）。
  一個會顯示假狀態的警報管道，比沒有警報更糟。

  修法方向：`_public_job()` 對 terminal 狀態的 job 也查一次 LightRAG 的 doc_status，
  兩者不一致時顯示實況並標記「外部改動過」。`foreign` 那條路已經證明查得到。

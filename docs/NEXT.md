...
- ⬜ 62 處偏微分符號誤讀　→ `scan-partial` 的漂移歸零

  `\hat{α}`／`\hat{O}`／`\hat{o}`／`\hat{D}` 等 9 種、62 處，全都應該是 `∂`。
  ⚠ **不能整批換**：`latex_fix` 的測試記著「`\hat{c}` 全母體 9 處裡有 3 處是真的
  遞迴係數 ĉ_n，換掉會毀資料」，還有一條「t 檢定的 σ̂ 是樣本標準差，改成 ∂ 就是
  毀資料」。要逐種看上下文才能決定哪幾種進白名單。

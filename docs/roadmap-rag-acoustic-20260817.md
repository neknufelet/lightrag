...
### 3.6 先確認備份真的能還原　⚠ **刪庫前沒做（PO 裁），刪庫後補驗了一半**

PO 2026-08-19 裁定舊庫那次不驗（「之前有人驗過了，我大概也不會回來了」）。
⚠ 紀錄與他的印象**對不上** —— `docs/NEXT.md` 一直標著 `(未驗)`。以紀錄為準寫在這裡。

**新庫起來、只有 3 份文件時補驗了本機還原點**（那是代價最低的時刻）：

```
文件 3 → 0、實體 943 → 0、收件匣 0 → 3、models 49 檔一個沒少
```

做法與逐項證據在 [backup-design](backup-design.md)。
⚠ **異地那份（restic → Google Drive）仍然沒有人驗過** —— 兩者防的災難不同，
不能互相代替。

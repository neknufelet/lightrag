...
### 2026-08-07 實測結果：四條判準全部通過

| 步驟 | 結果 | 對照 |
|---|---|---|
| 解析 | 556 項目、43 秒、`is_bundle_valid` 通過 | 基準 556，**分毫不差** |
| 裁定對位 | **10/10** 索引仍是 `table`、頁碼吻合 | 預期最壞重判 10 張，實際成本 0 |
| 後處理 | 空表格 **9 → 1**、修補 73 項、消音 146 項（有原文備份） | 判準是「不比 9/57 多」 |
| 入庫 | 40 分鐘，`processed: 1/1` | 本機 LLM 序列跑 |
| **關係數** | **節點 1,239 / 邊 1,995** | 判準是「不是 0」。邊多於節點＝健康的圖 |
| 接地率 | 實體可疑 3.0%（17/1239）、關係兩端都對不到 0.4%（3/1995） | 逐一看內容：全是命名習慣與已知字形問題 |
| 掉字 | 10.6%（383/3629） | 歷史 10.5%（381/3629），差 2 詞＝0.055%，**重現不是退步** |
| 五個端點 | `docs` / `doc` / `figures` / `images` / `search` 全通 | `doc` 端點看得到「已修補」標記 |

**三個 skill 一行都沒改就能用**——新部署剛好落在同一個位址與 workspace，所以 D1 的
「凍契約不凍位址」這輪沒被考驗到，但契約本身驗過了。

**D6 那個沒人量過的問題有答案了。** 本機模型在表格轉錄會捏造 `<img src="https://i.imgur.com/…">`，
抽實體關係時會不會也編——**沒有**。3% 的可疑逐一看內容：

| 可疑實體 | 實際是什麼 |
|---|---|
| `Mechl`、`Region I`、`Region Ii`、`Region Iii` | **KI-003 的羅馬數字誤讀**，`model-observations` 記過「換模型仍然難」 |
| `Coefficient B0`、`Geometric Parameter A/T/S` | 模型自己加了型別前綴。`B0` 在原文裡有，`Coefficient B0` 這個字串沒有 |

⚠ 掉字那條**是 fail 且維持 fail**。10.5% 在 2026-08-02 已逐詞歸因：表格結構黏連 117 詞
（KI-001，要重排整表才能解，與定點補格互斥）、內容在圖裡 73 詞、bbox 未覆蓋 91 詞。
`coverage-check` 現在會自動印出這段前例（2026-08-07 補上，之前只有 `extract-check` 接了）。

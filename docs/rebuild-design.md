...
**不是為了方便，是因為 LightRAG 給不了。** 2026-08-07 對跑著的 v1.5.6 問它的端點清單，
關鍵在**沒有什麼**：

| skill 要什麼 | LightRAG `:9621` |
|---|---|
| 跨篇發現（搜片段） | ✅ `/query` |
| **單篇的章節、表格、公式、圖片清單** | ❌ 沒有任何端點 |
| **圖片本體** | ❌ 完全沒有 |

`/documents` 只回「有哪些文件、狀態如何」，不回文件裡面長什麼樣。**原因是那些資料在
MinerU 的產物裡**（`content_list.json`、`images/`），躺在磁碟上——LightRAG 從頭到尾
沒讀過那些檔案，它只拿到切好的文字片段。

所以這一層有三個理由，按重要性：

1. **它是兩個 skill 的唯一來源。** `lightrag-fetch` 與 `lightrag-images` 完全靠它。
   拿掉這層，「單篇原料」與「找圖」兩個用途直接消失。
2. **對外只開一個埠**（PO 當初做它的主要動機）。skill 的所有 curl 都只打 `:9700`。
3. 代持 API key，讓 skill 複製到任何機器都能用。實測 `:9621` 的 `/documents`
   不帶 key 回 403、帶 key 回 200，所以這條前提仍然成立
   （`/health`、`/api/version` 是刻意公開的，不能拿它們判斷）。

⚠ **第 2 點目前沒達成**：`:9621` 與 `:9700` 兩個都發佈到 Tailscale。2026-08-07 決定
**暫時保留 `:9621`**，因為 LightRAG 的 WebUI 有圖譜瀏覽器，對「拓展想法」可能有用。
之後再決定要不要關（見 `docs/NEXT.md`）。
...
**2026-08-07 實跑結果：10/10 全部對得上，成本是零。**
...
項目總數 556（2026-08-01 基準 556）
表格總數  57（基準 57）
空表格     9（基準 9）
```

判準是兩層：那個索引現在還是不是 `table`，以及頁碼跟 `review.md` 記的一不一樣。
只驗第一層不夠——可能剛好那個位置也有張表。

⚠ **這是一份文件的證據，不能推論到其他文件。** 鐵則第 8 條講的不可重現性是 MinerU 的
性質，這次沒發生不代表下次不會。擴到 20 篇時每一份都要重驗一次。

⚠ 來源 PDF 的 `sha256:1c7dcb0e1de0393c` 與 `records/ledger` 記的一致——**先確認是同一份
檔案**，索引對得上才有意義。順序不能顛倒。

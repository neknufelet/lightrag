...
| 三 抽取 vs 解析結果 | 抽出來的名字原文裡有沒有 | 有尺，🔴 **零呼叫點** |
| 四 查得到嗎 | 問了回得出東西嗎 | 有尺，🔴 零呼叫點 |

⇒ **真正在把關的只有第二層，而它只管表格與公式。**
散文抽出來的實體與關係對不對，**現在沒有任何人在看。**

⇒ 所以「接地檢查接進進料收尾」（要動的東西第 3 項）不是錦上添花，
**它是這一輪唯一會實質提高正確性把關的改動。**
...
| 3 | 接地檢查與 `graph-clean` 接進進料收尾 | 現在零呼叫點；重抽會長垃圾節點而沒人守 | 三 |
...
- [ ] **`title_block` 對拆好的單章要不要開火**（新庫會有大量拆章，見 ③）

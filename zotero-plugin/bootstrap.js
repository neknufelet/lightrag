...
//
// ⚠ **`_Raged` 的大小寫是有意義的，不要改成 `_RAGED`。** 0.2.0 用的是大寫，
//    而 PO 手打的是小寫，於是同一件事有兩個標籤名。合併時踩到更大的坑：
//    Zotero 的 `DELETE /tags?tag=X` **不分大小寫**，刪大寫那個會把小寫的
//    一起帶走（2026-08-14，163 筆），靠事先存的備份才救回來。
//    要收拾只能逐筆 PATCH。所以這裡跟手打的對齊，不要再分岔。
//

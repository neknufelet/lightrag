...
| `SYMBOL` | `SYMBOL-2`（用模型判 1,482 個） | 🔵 `SYMBOL-1` ✅ 完成——答案卷 `tests/symbol1-answer-key.json`；另長出 `SYMBOL-3`／`SYMBOL-4` |
...
### `SYMBOL-2` — 用模型判那 1,482 個　⬜（前置＝`SYMBOL-1`）
...
### `SCANNER-1` — 封閉掃描器進版控，變成會自己響的探針　⬜ **最優先的工程項**

...
#
# **v2 只跑 compat-check，不跑 canary。** canary 比的是規則基準，而 v2 的基準
# 要等階段 2 的規則實際跑過才建立得起來。現在跑等於拿 v155 的基準去對 v2 的
# 資料，每天噴一次假漂移——而每天都響的警報等於沒有警報。
...
printf '{"at":"%s","status":"%s","compat_rc":%d,"detail":"%s","v2_compat_rc":%d,"v2_detail":"%s"}\n' \
  "$ts" "$status" "$rc" "$CHECK_DIR/compat-$ts.json" "$v2_rc" "$v2_detail" \
  > "$CHECK_DIR/latest.json"

# 保留規則要涵蓋 v2 的檔名，否則 v2-* 只增不減，永遠不會被清掉。
find "$CHECK_DIR" \( -name 'compat-2*' -o -name 'canary-2*' -o -name 'v2-compat-2*' \) |
  sort | head -n -90 | xargs -r rm --

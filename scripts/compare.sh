...
#
#   ./lr15-compare.sh 'Equivalent' lr15big
#   ./lr15-compare.sh 'Conventions' lr15test
#
# 舊管線: deeptutor DB / workspace acoustics_books
# 新管線: lr15_test DB / workspace 由第二參數指定
set -euo pipefail
...
n=$(q lr15_test  "select count(*) from lightrag_doc_chunks where workspace='$NEW_WS' and file_path ilike '%$DOC%'")
row "chunks" "$o" "$n"
...
n=$(q lr15_test  "select coalesce(sum(length(content)),0) from lightrag_doc_chunks where workspace='$NEW_WS' and file_path ilike '%$DOC%'")
row "chunk 總字元" "$o" "$n"

# --- 含 AI 評註的 chunk ---
o=$(q deeptutor  "select count(*) from lightrag_doc_chunks where workspace='$OLD_WS' and file_path ilike '%$DOC%' and $COMMENT")
n=$(q lr15_test  "select count(*) from lightrag_doc_chunks where workspace='$NEW_WS' and file_path ilike '%$DOC%' and $COMMENT")
...
n=$(q lr15_test  "select count(*) from lightrag_doc_chunks where workspace='$NEW_WS' and file_path ilike '%$DOC%' and $MANGLED")
row "掉字 chunk" "$o" "$n"

# --- entities / relations ---
o=$(q deeptutor  "select count(*) from lightrag_vdb_entity where workspace='$OLD_WS' and file_path ilike '%$DOC%'")
n=$(q lr15_test  "select count(*) from lightrag_vdb_entity_text_embedding_3_small_1536d where workspace='$NEW_WS' and file_path ilike '%$DOC%'")
row "entities" "$o" "$n"

o=$(q deeptutor  "select count(*) from lightrag_vdb_relation where workspace='$OLD_WS' and file_path ilike '%$DOC%'")
n=$(q lr15_test  "select count(*) from lightrag_vdb_relation_text_embedding_3_small_1536d where workspace='$NEW_WS' and file_path ilike '%$DOC%'")
row "relations" "$o" "$n"
...
n=$(q lr15_test  "select coalesce(sum(content_length),0) from lightrag_doc_status where workspace='$NEW_WS' and file_path ilike '%$DOC%' and status='failed'")
row "被丟棄正文字元" "$o" "$n"

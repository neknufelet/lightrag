...
其餘五格要跑別的檢查，不在這支的範圍。**沒跑過的閘門一格都不填** ——
...
def main() -> int:
    ...
    planned: list[tuple[str, str, str, str]] = []      # (文件, 閘門, 三態, 理由)
    ...
    for doc in sorted(live):
        ...
        planned.append((doc, "pp.equations", "pass" if n == 0 else "fail",
                        f"∂ 誤讀探針（上下同形＋行內斜線）命中 {n} 處"))

    tally = Counter((g, s) for _, g, s, _ in planned)
    ...
    written = failed = 0
    for doc, gate, state, note in planned:
        try:

...
# **中日韓字元要留下來。**（2026-08-12 修）
# 原本是 `[^a-z0-9]+`，於是 `norm("微穿孔板") == ''` —— 中文整串被當成標點抹掉。
# 庫裡唯一那份中文文獻 233 個實體裡 95 個因此被判成「模型編出來的」，
# 而那些詞用 SQL 查得到就在原文裡。**表上那面紅燈在說謊。**
# 標點（、，．）仍然要抹掉，跟英文標點同一個待遇 —— 只留表意字與假名、諺文。
_NORM = re.compile(
    r"[^a-z0-9"
    r"぀-ヿ"      # 平假名、片假名
    r"㐀-䶿"      # 漢字擴充 A

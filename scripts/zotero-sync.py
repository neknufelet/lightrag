...
7. **kbapi 的 `/docs` 列的是「解析過的」，不是「圖譜裡有的」。** 它讀
   `work/parsed`，不讀 LightRAG 的資料庫。平常兩者一致，**刪掉文件之後就分岔**
   —— 2026-08-14 把一篇誤入庫的論文從圖譜刪掉，kbapi 照樣列著它。
   所以刪文件時要一併把 `work/parsed` 底下的解析包移走；不移的話除了這裡會
   誤判，那份 PDF 再進收件夾還會**命中快取跳過解析**，一路走到抽取。

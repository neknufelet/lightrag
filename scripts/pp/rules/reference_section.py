...
**⚠ 消掉參考清單不是損失，是決定**（PO 2026-08-16）。上面那條「佔檢索預算」
只是省空間的理由，**不是主要理由**。主要理由是：

    參考清單的格式沒有一份是照規矩的，裡面寫的字串也不一定連得到真正要的
    那篇文獻。要建立文獻之間的關聯，靠**內容圖譜**去連，不靠那些名字字串。

所以「知識庫回答不了『這篇引用了誰』」**不是待補的洞，是選擇的結果**。
要重新討論這件事，先推翻上面那個理由，不要從「有東西不見了」開始。
"""
...
#
# **2026-08-16 補上實測**（原本只有上面那句推測，沒有數字 —— 鐵則第 5 條
# 「門檻用量的不要用調的」要求有母體）。全庫 317 份，先把 `_pp_original_*`
# 還原回記憶體再跑 `plan()`（直接量已消音的資料會得到假的 0）：
#
#     C 組（單篇論文）178 份　中位 13.7%　75% 18.0%　90% 22.1%　95% 25.5%
#     B 組（拆章書）  121 份　中位  0.0%　最高 10.2%
#     A 組（Mechel）   18 份　中位  0.6%　最高  6.4%
#
# ⇒ 上面那句「常佔 10–15%」猜對了（中位 13.7%），30% 落在約第 98 百分位。
# 全庫只有 3 份超過，最高的是 `2025 - Recent advancements in sound-absorbing
# materials` 的 **49.0%** —— 那是**綜述論文**，參考清單本來就巨大。
# **它該被擋下來讓人看，不是規則壞了。**
#
# ⚠ 新庫只有論文，所以 C 組的分佈就是全庫分佈。門檻仍然適用，但**重抽後要重量** ——
# 這個數字綁的是語料組成，不是規則。

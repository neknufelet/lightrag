...
> ⚠ **2026-08-16 當天稍晚修掉兩個缺陷**：canary 的報告檔原本會被同一支腳本
> 刪掉（清理步驟按檔名排序，`canary-` 字母序最前），而且 `latest.json`
> **沒有 canary 這一欄**。現在清理改按修改時間排（`scripts/prune-checks.sh`），
> `latest.json` 也補上 `canary_rc`。下面那份是修之前的紀錄，所以看不到 canary。
...
| `fresh_rc=2` | 跑著的不是最新碼 | **信**。dker 落後版控。當天稍晚同步完轉綠，重跑實測 `fresh_rc=0` |
...
- ✅ **腳本誰叫誰、讀寫哪些檔** —— [script-map.md](script-map.md)
